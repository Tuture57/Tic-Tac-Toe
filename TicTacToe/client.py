...
#La config des données pour se connecter au serveur
HOST_ADDR = "127.0.0.1"
HOST_PORT = 8432
...
def gameReset():
    ...
    #Force le joueur avec les croix à commencer à jouer
    if a == "X":
...
#Fonction pour vérifier la victoire de l'adversaire
def checkWin2():
    global a, advteam, tours
    for i in range(3):
        #vérifie pour la meme ligne ou colonne
...
def buttonClick(row,col):
    ...
    #désactive le clic car ca passe au tour de l'adversaire
    click_disable()
    label.config(text = "C'est au tour de l'adversaire "+ advteam)
    checkWin()
